...
class AdminControllersTest {
    ...
    private static final String adminUsername = "test_admin";
    private static final String adminPassword = "yolo";
    private static final String player1Username = "player1";
    private static final String player1Password = "password";
    ...
    void setup() {
        ...
        String playerHash = passwordEncoder.encode(player1Password);
        jdbcTemplate.update(
            "INSERT INTO players (name, password, role, handicap, team_id) VALUES (?, ?, ?, ?, ?) ON CONFLICT (name) DO NOTHING",
    ...
    private static Stream<Arguments> provideCreateParameters() {
        return Stream.of(
                // Team provided success
                Arguments.of(
                ...
                // success when team is not provided
                Arguments.of(
                        adminUsername, adminPassword,
                        path,
                        """
                ...
                // failure on conflict of player name
                Arguments.of(
                        adminUsername, adminPassword,
                        path,
                        """
    ...
    @ParameterizedTest
    @MethodSource("provideCreateParameters")
    void createPlayer_Tests(String username, String password, String apiPath, String payload, Object expected,
                            HttpStatus expectedStatus, boolean checkPersistence)
            throws JsonProcessingException {
        String token = loginAndGetToken(username, password);

        ResponseEntity<String> response = sendRequest(apiPath, payload,
                token,
                HttpMethod.POST, restClient);

        assertEquals(expectedStatus, response.getStatusCode(), "Http status code should be " +
    ...
    @ParameterizedTest
    @MethodSource("provideReadParameters")
    void getPlayers_Tests(String username, String password, String apiPath, Object expected,
                          HttpStatus expectedStatus) throws JsonProcessingException {
        String token = null;
        try {
            token = loginAndGetToken(username, password);
        } catch (Exception e) {
            if (expectedStatus == HttpStatus.UNAUTHORIZED && e instanceof HttpClientErrorException.Unauthorized) {
                return;
            } else {
                throw e;
            }
        ...
        ResponseEntity<String> response = sendRequest(apiPath, null,
                token,
    ...
    // TODO Make these tests parameterised and use tokenized calls
//    // ------------------- Update tests --------------------
//
...
//
    ...
    private String loginAndGetToken(String username, String password) throws JsonProcessingException {
        String loginJson = objectMapper.writeValueAsString(new AuthRequestDTO(username, password));
